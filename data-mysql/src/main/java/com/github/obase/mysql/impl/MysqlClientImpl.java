...
import static com.github.obase.base.StringBase.isNotEmpty;
...
public class MysqlClientImpl extends MysqlClientOperation {
	...
	protected void doInit(Connection conn) throws Exception {
		...
		if (isNotEmpty(packagesToScan)) {
			...
			for (String pkg : pkgs) {
				if (isNotEmpty(pkg)) {
					sb.setLength(0);
					String packageSearchPath = sb.append(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX).append(ClassBase.getInternalNameFromClassName(pkg)).append("/**/*.class").toString();
					...
					for (Resource rs : rss) {
						...
						if (classMetaInfo.tableAnnotation != null || classMetaInfo.metaAnnotation != null) {
							metaMetaInfoMap.put(ClassBase.getClassNameFromInternalName(classMetaInfo.internalName), classMetaInfo);
					...
					for (Resource rs : rss) {
						...
						for (Statement stmt : configMetaInfo.statementList) {
							...
							if (statementCache.put(key, stmt) != null) {
								throw new SystemException(MysqlErrno.SQL_CONFIG_DUPLICATE, "Duplicate statement id: " + key);
							}
						}
					}
				}
		...
		for (Map.Entry<String, ClassMetaInfo> entry : metaMetaInfoMap.entrySet()) {
			...
			clazz = ClassBase.forName(entry.getKey());
	...
	public <T> List<T> selectList(Class<T> table) throws SQLException {
		...
		if (pstmt == null) {
			throw new SystemException(MysqlErrno.META_INFO_NOT_FOUND, "Not found table: " + table);
		}
	...
	public int execute(String updateId, Object params) throws SQLException {
		...
		if (xstmt == null) {
			throw new SystemException(MysqlErrno.META_INFO_NOT_FOUND, "Not found statement: " + updateId);
		}
	...
	public <T> int[] batchExecute(String updateId, List<T> params) throws SQLException {
		...
		throw new SystemException(MysqlErrno.SQL_DYNAMIC_NOT_SUPPORT, "executeBatch don't support dynamic statement: " + xstmt.id);
	}
